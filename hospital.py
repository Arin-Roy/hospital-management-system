...
class hsptl():
    def __init__(self, root):
        ...
        diseaseLbl = tk.Label(admitFrame, text="Disease:", bg=self.clr(150,150,230), font=("Arial",15,"bold"))
        diseaseLbl.grid(row=3, column=0, padx=20, pady=15)
        self.disease = tk.Entry(admitFrame, width=18, bd=3, font=("arial",15,"bold"))
        self.disease.grid(row=3, column=1, padx=10, pady=15)
    ...
    def tabFun(self):
        ...
        self.table = ttk.Treeview(tabFrame,xscrollcommand=x_scrol.set, yscrollcommand=y_scrol.set,
                                  columns=("pid","name","bg","disease","points","med","addr"))
    ...
    def admitFun(self):
        ...
        if id and name and bg and disease and p and med and addr:
            point = int(p)
            try:
                self.dbFun()
                self.cur.execute("insert into hospital(id,name,b_group,disease,points,medics,addr) values(%s,%s,%s,%s,%s,%s,%s)",(id,name,bg,disease,point,med,addr))
                self.con.commit()
    ...
    def medFun(self):
        ...
        try:
            ...
            row = self.cur.fetchone()
            if row:
                tk.messagebox.showinfo("Medicine", f"Recommended Medicine for Patient is/are:{row[0]}")
                self.con.close()
            else:
                tk.messagebox.showerror("Error",f"Invalid Patient ID:{id}")
        except Exception as e:
    ...
    def pointFun(self):
        ...
        try:
            ...
            if row:
                ...
                self.con.commit()
                tk.messagebox.showinfo("Success","Health Points Updated!")
                self.desFrame()
                self.cur.execute("select * from hospital where id=%s",id)
    ...
    def desFun(self):
        ...
        try:
            ...
            self.con.commit()
            tk.messagebox.showinfo("Success", f"Patient with id:{id} discharged successfully!")
            self.table.delete(*self.table.get_children())
            self.con.close()
    ...
    def dbFun(self):
        self.con = pymysql.connect(host="localhost", user="root", passwd="22052803", database="new_schema")
